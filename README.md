...
3. 근무 일정 추가 (UPDATE)
  ...
  - 출근 버튼 클릭 시 당일 일급 추가, 근무지 블록 컴포넌트의 월급 값이 변경되면서 리렌더링 (React Redux, useState, Sequelize 사용)
...
- 직접 추가 버튼 클릭 시 선택한 날짜의 일급 추가, 근무지 블록 컴포넌트의 월급 값이 변경되면서 리렌더링 ("react-multi-date-picker" 라이브러리, React Redux, useState, Sequelize 사용)
